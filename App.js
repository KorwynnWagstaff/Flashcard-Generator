...
const initialCards$ = Rx.Observable.of([
    new BasicCard('Who was the last president of the United States?', 'Donald Trump'),
    new ClozeCard('Pluto use to be a planet.', 'Pluto')
  ]);
...
answers$.subscribe(ans => {
  ...
  switch (ans.name) {
    case 'action': {
      ...
      else return displayCards$.onNext('Crazy');
    }
